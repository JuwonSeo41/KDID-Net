...
class Predictor:
    def __init__(self, weights_path: str, model_name: str = ''):
        with open('config/KD_config.yaml',encoding='utf-8') as cfg:
            config = yaml.load(cfg, Loader=yaml.FullLoader)
        model = get_generator(model_name or config['model'])
        model.load_state_dict(torch.load(weights_path)['model'])
        print('It''s %d epoch weights' % torch.load(weights_path)['epoch'])
    ...
    def __call__(self, img: np.ndarray, mask: Optional[np.ndarray], ignore_mask=True) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            if not ignore_mask:
                inputs += [mask]
            pred, *_ = self.model(*inputs)  # pred = restored img, logits = classification(Teacher model)
            # pred = self.model(*inputs)     # Small model has no logits for classification
