...
from models.fpn_inception_small import FPNInception
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc=3, output_nc=3, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, use_parallel=True, learn_residual=True, padding_type='reflect'):
        ...
        for i in range(n_downsampling):
            mult = 2**i
            model = model + [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3,
        ...
        for i in range(n_blocks):
            model = model + [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]

        for i in range(n_downsampling):
            mult = 2**(n_downsampling - i)
            model = model + [nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
        ...
        model = model + [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model = model + [nn.Tanh()]
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block = conv_block + [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias),
                                   norm_layer(dim),
                                   nn.ReLU(True)]
        if use_dropout:
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=1, norm_layer=nn.BatchNorm2d, use_sigmoid=False, use_parallel=True):
